...
export const upsertSpecProposalChangeFn = createServerFn({ method: "POST" })
	...
	.handler(async ({ data }) =>
		withAuthUser(async () => {
			...
			const frontmatter = formValuesToFrontmatter(data.specLevel, data.values);
			const fmCheck = validateFrontmatterForLevel(data.specLevel, frontmatter);
			if (!fmCheck.ok) {
