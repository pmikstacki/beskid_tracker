...
async function discoverAuthHub() {
	...
	try {
		...
		const client = new BeskidAuthClient({
			baseUrl: hubUrl,
		}) as BeskidAuthClient & {
...
async function discoverLegacyHub(hubUrl: string) {
	...
	if (!appsResponse.ok || !pairingResponse.ok)
		throw new Error("Auth hub discovery failed");
	...
	return {
		app: app ? { label: app.label ?? "Beskid Tracker" } : null,
		pairing: { paired: pairing.paired === true },
	};
}
