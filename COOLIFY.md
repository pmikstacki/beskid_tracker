...
3. Open the pairing link from the hub (or **Settings → Auth pairing**). When `TRACKER_PUBLIC_URL` is set, the link auto-approves using `TRACKER_PAIRING_APPROVER_LOGIN` or `GITHUB_SYNC_TOKEN` (no sign-in required). Otherwise sign in as a repo admin and submit the code.
