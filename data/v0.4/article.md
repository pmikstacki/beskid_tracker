...
| **v04-corelib-complete** | Compiler structure; 67-target matrix; API/runtime completion | In progress — dated 20/61 and 38/61 baselines are retained; fresh 67/67, collection, Core.FS, native-kit, and publish evidence remain |
...
Historical W0–W6 wave work is present, but current source-tip release evidence does not support calling the compiler/Corelib/runtime complete. On 2026-08-09 the release CLI built and the explicit runtime kit staged, while the then-current EOF-bound Corelib matrix finished at 20/61 passing. A later reconstruction covered that 61-target manifest at 38 passing / 23 failing. Corelib `ce8bc7f` added six production targets, so the current release denominator is 67 and requires fresh 67/67 evidence. The historical apparent hang was stdin job control, not a compiler deadlock. Structural splits completed for the semantic contract, ISLE library, package store, GC, TypeChecker expressions, and module emission; remaining qualified monoliths are tracked separately.

**Remaining for 0.4 sign-off:** recover 67/67 Corelib and harness performance; complete semantic/enum/generic lowering gaps; implement rooted Core.Collections and canonical host-backed Core.FS; retire all Corelib stubs; correct per-artifact runtime-kit verification; obtain fresh Linux, macOS arm64, and Windows x86-64 debug/release static/shared JIT+AOT evidence; finish tracker/release sign-off.
